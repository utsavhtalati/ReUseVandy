...
export async function getSavedPBListings() {
  try {
    const records = await pb.collection("saved").getFullList({
      sort: "-created",
      expand: "ListingID",
    });

    const enrichedRecords = records.map((record) => {
      // Access the expanded 'ListingID' object, if it exists
      const listingDetails = record.ListingID || null;

      // Return a new object with all original record data plus the listing details
      return {
        ...record, // Spread the original record data
        listingDetails: listingDetails, // Add the listing details
      };
    });
...
export async function saveListing(listingId: any) {
  try {
    // First, check if the listing is already saved
    const existingListings = await pb.collection("saved").getList(1, 1, {
      filter: `ListingID='${listingId}'`,
    });

    if (existingListings.items.length > 0) {
      // If the listing already exists, log a message and stop further execution
      console.log("This listing has already been saved.");
      return "This listing has already been saved.";
    }

    // If the listing does not exist, create a new record
    const newSavedListing = await pb.collection("saved").create({
      ListingID: listingId,
      // ... any other fields you want to set for the "saved" record
    });

    console.log("Listing saved successfully:", newSavedListing);
